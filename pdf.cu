...
----------------------------------------------------------------------*/
  ...
  for (int a=0; a<3; a++) alth[a] = 0.5*al[a];
  /* Max. pair distance RHMAX & histogram bin size DRH */
  float rhmax = sqrt(alth[0]*alth[0]+alth[1]*alth[1]+alth[2]*alth[2]);
  float drh = rhmax/NHBIN;  // Histogram bin size

  nhis = (float*)malloc(sizeof(float)*NHBIN);
  // for (int ih=0; ih<NHBIN; ih++) nhis[ih] = 0.0; // Reset the histogram
  ...
  float density = n/(al[0]*al[1]*al[2]);
  /* Print out the histogram */
  fp = fopen("pdf.d","w");
  for (int ih=0; ih<NHBIN; ih++) {
    float gr = nhis[ih]/(2*M_PI*pow((ih+0.5)*drh,2)*drh*density*n);
    fprintf(fp,"%e\t%e\n",(ih+0.5)*drh,gr);
  }
  fclose(fp);
  free(nhis);
...
/*--------------------------------------------------------------------*/
  ...
  float cpu1 = ((float) clock())/CLOCKS_PER_SEC;
  histogram();
  float cpu2 = ((float) clock())/CLOCKS_PER_SEC;
